...
rectangle intentionally catches all of Germany, but it also includes parts of
neighboring countries. This resolver therefore does not assign every point to
the nearest German state. It uses three explicit categories:
...
# ASCII spellings matching LAND_TO_FOLD in the calibration/modeling scripts.
EXPECTED_LAENDER = {
    "Baden-Wuerttemberg",
    "Bayern",
    "Berlin",
...
def transliterate_umlauts(value: object) -> str:
    """Return the ASCII Land spelling used by downstream fold mappings."""
...
def detect_name_column(gdf: gpd.GeoDataFrame) -> str:
    """Find the boundary column that contains the 16 German Land names."""
...
def discover_months(processed_dir: Path) -> list[str]:
    """Return available SDS011 node months, ignoring macOS sidecar files."""
...
def load_sensor_coords(processed_dir: Path, months: list[str]) -> tuple[pd.DataFrame, dict]:
    """Load SDS011 node coordinates and collapse month-to-month GPS jitter."""
...
def load_land_polygons(boundaries_path: Path, name_column: str | None) -> gpd.GeoDataFrame:
    """Load valid German Land polygons in WGS84 with normalized names."""
...
def assign_land(
    ...
    """Assign German-state labels without forcing foreign points into Germany.

    Direct assignment is done in WGS84 because both the Sensor.Community node
    coordinates and the GeoJSON boundary file are longitude/latitude data.
    Distances are measured only after projecting to EPSG:3035; this avoids using
    angular degrees as if they were meters. Points outside all polygons are kept
    in the diagnostics, but only points within the boundary tolerance are written
    to sensor_land.csv.
    """
    ...
    # `covered_by` keeps points that fall exactly on a state boundary or coast
    # with the polygon instead of treating them as outside.
    ...
    if not unmatched.empty:
        ...
        # The fallback is intentionally narrow. The audit found the large tail of
        # unmatched points are in neighboring countries admitted by the upstream
        # rectangular Germany bounding box, not boundary precision noise.
        fallback_mask = (
...
def distance_bucket_counts(diagnostics: pd.DataFrame) -> dict[str, int]:
    """Bucket distances for sensors not directly covered by a Land polygon."""
...
def write_plot(
    ...
    """Create a compact diagnostic map of direct, fallback, and excluded points."""
...
def write_json(path: Path, obj: dict) -> None:
    """Write deterministic JSON, creating the parent directory first."""
