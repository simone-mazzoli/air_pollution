...
def make_map(df, pollutant, path):
    ...
    sc = ax.tricontourf(df["lon"], df["lat"], df[f"pred_{pollutant}"], levels=20, cmap="viridis")
    ax.scatter(df["lon"], df["lat"], s=1, c="black", alpha=0.15)
...
def parse_args():
    ...
    ap.add_argument("--out", default="grid_predictions.csv")
    ap.add_argument("--plot", default="grid_map.png")
...
def main():
    ...
    out_name = f"{model_config['experiment']}_{args.out}"
    out.to_csv(out_name, index=False)
    print(f"saved {out_name}")

    plot_name = f"{model_config['experiment']}_{args.plot}"
